...
define(['N/http',
        ...
        'N/runtime'], function (http, ui, search, task, runtime, serverWidget) {
            ...
            function showResults(context, requstedPeriod, intCustName) {
                ...
                }).defaultValue = context.request.parameters.custpage_act_period;
                var custName = objForm.addField({
                    id: 'custpage_customer',
                    type: ui.FieldType.SELECT,
                    label: 'Customer',
                ...
                var invSearch = search.create({
                    ...
                    filters: [
                                       ...
                                       search.createFilter({
                                           name: 'postingperiod',
                                           operator: search.Operator.ANYOF,
                                           values: requstedPeriod
                                       }),

                                   cusFilter,
                    ],
                    columns: ['internalid', 'name', 'tranid', 'amount', 'trandate']
                });

                var myBal = invSearch.run().getRange({
                    start: 0,
                    end: 100
                });
