...
function scheduled_DeleteProcessedATG(type) {
    ...
    try {
        ...
        arrFilters.push(new nlobjSearchFilter('internalid', null, 'greaterthan', intId));


        var arrProcessedATG = SuiteUtil.search(stATGProcessedSavedSearch, 'customrecord_csi_atg_settlement', arrFilters, arrColumns);
...
function deleteProcessedATGSettlement(arrProcessedATG) {
    ...
    for (var intCount = 0; intCount < intProcessedATGCount; intCount++) {
        try {
            ...
            var isProc = getProcessedSalesOrder(orderID, transDetails, transType, processType);
            if (isProc == 'T') {
...
function getProcessedSalesOrder(orderid, shippinggroup, transType, processType) {
    var arrFilters = [];
