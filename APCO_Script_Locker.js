 ...
 * 1.10 	  	07 Apr 2017 	  	ajdeleon 		Added queues left for multi queue
 */
...
var ScriptLocker =
    {
        ...
        CUSTRECORD_SCRIPT_LOCKER_QUEUES_LEFT: 'custrecord_script_locker_queues_left',
        CURRENT_SCRIPT_RECORD_ID: '',
        ...
        */
        ...
        remove: function (option) {
            ...
            if (option.id) {
                //get current queue
                var intQueuesLeft = nlapiLookupField(ScriptLocker.CUSTOMRECORD_SCRIPT_LOCKER, option.id, ScriptLocker.CUSTRECORD_SCRIPT_LOCKER_QUEUES_LEFT);

                //convert to int
                intQueuesLeft = parseInt(intQueuesLeft, 10);
                if (isNaN(intQueuesLeft) || (intQueuesLeft == Infinity)) {
                    intQueuesLeft = 0;
                }

                //deduct queue
                intQueuesLeft = (intQueuesLeft - 1);


                if (intQueuesLeft > 0) {
                    //save to current script lock id
                    nlapiSubmitField(ScriptLocker.CUSTOMRECORD_SCRIPT_LOCKER, option.id, ScriptLocker.CUSTRECORD_SCRIPT_LOCKER_QUEUES_LEFT, intQueuesLeft);
                    nlapiLogExecution('audit', logTitle, 'SCRIPT_LOCKER_REMOVE_QUEUE: id=' + option.id + ' queues left: ' + intQueuesLeft);
                }
                else {
                    //delete record
        ...
        isOnProcess: function (option) {
            ...
            //v1.10
            if (results) ScriptLocker.CURRENT_SCRIPT_RECORD_ID = results[0].getId();
        ...
        /**
         * @param {String} scriptLockerId REQUIRED
         * v1.10
        **/
        addQueuesLeft: function (scriptLockerId) {

            var logTitle = 'ScriptLocker.addQueuesLeft';

            // Check required parameters
            ScriptLocker.requireParam(scriptLockerId);

            //get current queue
            var intQueuesLeft = nlapiLookupField(ScriptLocker.CUSTOMRECORD_SCRIPT_LOCKER, scriptLockerId, ScriptLocker.CUSTRECORD_SCRIPT_LOCKER_QUEUES_LEFT);
            ...
            //validate
            if (intQueuesLeft < 0) intQueuesLeft = 0;

            //add queue
            intQueuesLeft = (intQueuesLeft + 1);

            //save to current script lock id
            nlapiSubmitField(ScriptLocker.CUSTOMRECORD_SCRIPT_LOCKER, scriptLockerId, ScriptLocker.CUSTRECORD_SCRIPT_LOCKER_QUEUES_LEFT, intQueuesLeft);

            nlapiLogExecution('audit', logTitle, 'SCRIPT_LOCKER_ADD_QUEUE: id=' + scriptLockerId + ' queues left: ' + intQueuesLeft);
        },
