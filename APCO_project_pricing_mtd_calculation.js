...
function cumProjectAmounts(type) {
    if (type == 'edit') {
        ...
        if (recType == 'custrecord_apco_pricing_project') {
            //projectId = nlapiGetFieldValue("custrecord_apco_project")
            projectId = nlapiLookupField('custrecord_apco_pricing_project', projectId, 'custrecord_apco_pricing_project');
        }
        ...
        arrFilters.push(new nlobjSearchFilter('custrecord_apco_pricing_project', null, 'is', projectId));
        ...
        var arrREQ = nlapiSearchRecord('custrecord_apco_pricing_project', 'customsearch_apco_ud_cumulative', arrFilters, arrColumns)
        if (arrREQ) {
            ...
            for (var i = 0; i < arrREQ.length; i++) {
                ...
                bTotal = parseFloat(bTotal) + parseFloat(arrREQ[i].getValue('custrecord_budget_value_pp'));
               // oopTot = parseFloat(oopTot) + parseFloat(arrREQ[i].getValue('custrecord_apco_rev_oop'));
                //update this row in the custom record
                updateCumulativeAmounts(intId, bTotal);
            }
        }
    }
}

function updateCumulativeAmounts(intID, bfTot) {
